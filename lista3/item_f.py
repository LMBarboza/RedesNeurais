...
def main() -> None:
    ...
    model = RedeFactory.createRede(
        sChannels, sOutput, hiddenLayers, sKernel, fnActivation=nn.ReLU, dropout=True
    ).to(device)
    ...
    torch.save(model.state_dict(), f"models/model_dropout.pth")

    with open("results/drouput_accuracy.json", "w") as f:
